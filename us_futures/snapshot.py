"""
US Futures direct fetch via yfinance — no Redis or DB dependency.

Modes
-----
snapshot (default)
    Batch-fetch last_price / last_volume for all symbols.
    Output: JSON array  [{symbol, last_price, last_volume}, ...]

ohlcv --symbol <SYM> [--limit N]
    Daily OHLCV bars for one symbol (default 200 bars).
    Output: JSON array  [{time, open, high, low, close, volume}, ...]
"""
...
SYMBOLS = ["ES=F", "NQ=F", "YM=F", "CL=F", "GC=F", "SI=F", "ZB=F"]
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="US Futures yfinance fetcher")
    sub = p.add_subparsers(dest="mode")
    sub.add_parser("snapshot", help="Live snapshot for all symbols")
